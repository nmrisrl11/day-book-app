...
- **Global Search**: A highly optimized, keyboard-first Command Palette (`⌘K`) built with `cmdk`. It allows users to instantly search and navigate to application pages, specific settings tabs, and individual person profiles without rendering thousands of DOM nodes. It is also accessible via a search icon in the primary header.
