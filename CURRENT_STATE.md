...
- **Data Model**: Birthdays act as "Person Cards" with an ID, Name, Date, Avatar (Base64 or external ID), Relationship (Me, Family, Friend, Partner, Colleague, Other), Notes (up to 5 lightweight string tags), and Gift Ideas/Wishlists (up to 10 strings). The "Me" relationship is strictly restricted to a single profile per database. The Me profile's avatar is persistently displayed in the global navigation header for quick access, and duplicates introduced via JSON imports or P2P Sync are automatically converted to "Other" to maintain data integrity. Custom uploaded avatars are intelligently converted and optimized as WebP images to save IndexedDB space. The app natively supports precise age calculation for babies with fractional ages ("months old", "days old", "Newborn").
...
- **Navigation Structure**: Features a responsive navigation design. On desktop, the primary header strictly surfaces high-frequency features (Birthdays, Invitations, Settings) alongside the Global Search trigger and Notifications. On mobile devices (< 768px), these primary features seamlessly transition into a fixed Bottom Navigation Bar to maximize screen space, while keeping the top header clean. Secondary application routes (About, Install App) and global actions (Sync Data) are deliberately housed in the global footer to prevent cognitive overload.
...
Settings has grown into a full `/settings` route with 7 tabs managed by `nuqs` URL state, featuring a modern, cohesive, and fully responsive layout across all sections:

1.  **Appearance**: Theme toggle (Light/Dark) and Display settings (Upcoming count limit, animations, quick actions).
2.  **Avatar**: Choose between `boring-avatars` (default) and `avvvatars`, and enable/disable custom image uploads.
3.  **Main Greeting**: Customize the dashboard "Happy Birthday" text, typography, and color styling (Solid/Gradient).
4.  **Notifications**: Toggle in-app birthday reminders, customize how many days in advance to be reminded, and enable/disable native OS notifications via Service Worker (delivered only while the app is running). The app also features automatic update notifications that alert users via the in-app notification menu and OS-level notifications whenever a new version is released.
5.  **Messages & Greetings**: Manage floating text items, enable custom greetings, and define personalized greeting messages for birthdays.
6.  **Sound & Feedback**: Configure `cuelume` hover/click/success/error sound mappings and volume. UI feedback notifications use `goey-toast` for playful, premium animations.
7.  **Data Management**: Features a Storage Overview section to monitor browser data usage and enable Persistent Storage protection. **Device Sync (P2P)** enables direct, secure local network synchronization of the entire database (birthdays, invitations, settings) via WebRTC (using PeerJS Cloud for connection signaling and public TURN servers for traversal), featuring granular visual feedback during the handshake phase to reduce connection anxiety, and an audio confirmation on successful transfer. Both Device Sync and JSON settings exports strictly isolate and strip out device-specific configuration (like onboarding state and last seen version) to prevent cross-device state conflicts. All Import/Export functions (Birthdays, Calendar, Invitations, Settings) are consolidated into a compact list layout. The virtualized Import Preview Dialog handles duplicate reviewing and selective importing smoothly for 1,000+ items. **Settings Import Preview** allows users to visually diff incoming settings against current settings before applying them. Finally, the Danger Zone provides explicitly separated destructive actions (Delete All Birthdays, Delete All Invitations, Reset All Settings) each guarded by a generic ActionConfirmationModal and export-first safety hints.
...
- **System Notifications**: Natively supported via the Workbox Service Worker (`custom-sw.js`), hooking into the Notification API to send OS-level reminders. Because this is a local-first application, notifications are delivered only while the app is running; background delivery when the app is completely closed is not supported. Users can click notifications to seamlessly focus or reopen the DayBook application.
