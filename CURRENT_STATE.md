...
- **CRUD & Bulk**: Create, Read, Update, Delete functionality for birthdays. Users can select multiple records in the Management screens to assign relationships in bulk or perform bulk deletions (both for Birthdays and Invitation Links). The Management lists use virtualization (`@tanstack/react-virtual`) to effortlessly handle thousands of records without freezing.
