...
4.  **Notifications**: Toggle in-app birthday reminders, customize how many days in advance to be reminded, and enable/disable native OS push notifications via Service Worker (delivered only while the app is running). The app also features automatic update notifications that alert users via the in-app notification menu and OS-level push notifications whenever a new version is released.
...
- **System Push Notifications**: Natively supported via the Workbox Service Worker (`custom-sw.js`), hooking into the Notification API to send OS-level reminders. Because this is a local-first application, notifications are delivered only while the app is running; background delivery when the app is completely closed is not supported. Users can click notifications to seamlessly focus or reopen the DayBook application.
