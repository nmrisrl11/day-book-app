...
4.  **Visitor Tracker**: Uses Upstash Redis via Vercel Edge (`api/visitors.ts`). It employs IP-based locking (`SET NX EX 86400`) to prevent spam/refresh inflation. IP addresses MUST be anonymized via one-way SHA-256 hashing before reaching the database to preserve the privacy-first architecture. It safely bypasses network requests completely during local development (`import.meta.env.DEV`).
